...
/**
 * Busca a chave de API do Groq da coleção "ape_key"
 * @returns Promise com a chave de API ou null se não encontrada
 */
export const getGroqApiKey = async (): Promise<string | null> => {
  try {
    const apiKeyRef = collection(db, "ape_key");
    const q = query(apiKeyRef, limit(1));
    const snapshot = await getDocs(q);

    if (snapshot.empty) {
      console.error("[v0] Nenhuma chave de API encontrada na coleção 'ape_key'");
      return null;
    }
    ...
    if (!docData.groq) {
      console.error("[v0] Campo 'groq' não encontrado na coleção 'ape_key'");
      return null;
    }
...
/**
 * Busca qualquer chave de API da coleção "ape_key"
...
export const getApiKey = async (fieldName: string): Promise<string | null> => {
  try {
    ...
    if (!docData[fieldName]) {
      console.error(`[v0] Campo '${fieldName}' não encontrado na coleção 'ape_key'`);
      return null;
    }
