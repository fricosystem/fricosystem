...
export function PCPCharts({ period }: PCPChartsProps) {
  ...
  // Dados para gráfico de performance por classificação
  const dadosPerformanceClassificacao = useMemo(() => {
    if (!pcpData || pcpData.length === 0) return [];
    
    const classificacaoMap: Record<string, { planejado: number; produzido: number }> = {};
    
    pcpData.forEach(item => {
      const classificacao = item.classificacao || 'Sem classificação';
      if (!classificacaoMap[classificacao]) {
        classificacaoMap[classificacao] = { planejado: 0, produzido: 0 };
      }
      classificacaoMap[classificacao].planejado += item.quantidade_planejada || 0;
      classificacaoMap[classificacao].produzido += item.quantidade_produzida || 0;
    });

    return Object.entries(classificacaoMap)
      .map(([name, data]) => ({
        name,
  ...
  // Dados para gráfico de produção por setor
  const dadosProducaoPorSetor = useMemo(() => {
    if (!chartData.setoresChart || chartData.setoresChart.length === 0) return [];
    
    return chartData.setoresChart
      .map((item: any, index: number) => ({
        name: item.setor || 'Outros',
        value: item.producao_real || 0,
        fill: COLORS[index % COLORS.length]
      }))
      .sort((a: any, b: any) => b.value - a.value)
      .slice(0, 6);
  }, [chartData.setoresChart]);
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 mb-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Planejado vs Produzido por família</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dadosPerformanceClassificacao} layout="vertical">
                  <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
                  ...
                  />
                  <Legend />
                  <Bar dataKey="planejado" name="Planejado" fill="#3B82F6" radius={[0, 4, 4, 0]} />
                  <Bar dataKey="produzido" name="Produzido" fill="#10B981" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Factory className="h-5 w-5" /> Produção por Setor
            </CardTitle>
          ...
          <CardContent>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dadosProducaoPorSetor} layout="vertical">
                  <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
                  <XAxis type="number" tickFormatter={(value) => value.toLocaleString()} />
                  <YAxis dataKey="name" type="category" width={100} tick={{ fontSize: 11 }} />
                  <Tooltip {...tooltipStyle} formatter={value => [`${Number(value).toLocaleString()} kg`, 'Produção']} />
                  <Bar dataKey="value" name="Produção" radius={[0, 4, 4, 0]}>
                    {dadosProducaoPorSetor.map((entry: any, index: number) => (
                      <Cell key={`cell-${index}`} fill={entry.fill} />
                    ))}
                  </Bar>
                </BarChart>
