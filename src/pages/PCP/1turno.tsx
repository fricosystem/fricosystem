...
const PrimeiroTurno: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const localStorageKey = `primeiroTurno_${today}`;
      ...
      try {
        ...
        if (docSnap.exists() && docSnap.data()["1 Turno"]) {
          const firestoreData = docSnap.data()["1 Turno"].map((p: any) => ({
            codigo: p.codigo,
  ...
  const saveToFirestore = async () => {
    ...
    try {
      ...
      toast({
        title: "Sucesso",
        description: "Dados do 1° Turno salvos com sucesso!",
      });
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold">1° Turno</h2>
      
      <Card>
        <CardHeader>
          <CardTitle className="flex justify-between items-center">
            <span>Controle de Produção - 1° Turno</span>
