...
const EmptyChart = ({ message = "Sem dados registrados" }: { message?: string }) => (
...
export function ManutencaoCharts({ period }: ManutencaoChartsProps) {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      execucoesRealizadas: historicoExecucoes.filter(e => {
        const d = convertTimestamp(e.dataExecucao);
        return d ? filterByPeriod(d) : false;
      }).length,
  ...
  }, [tarefasManutencao, historicoExecucoes, paradasMaquina, manutentores, period]);
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4 mb-6">
        <StatsCard 
          ...
          value={formatNumber(stats.tarefasPendentes)} 
          icon={<Wrench className="h-5 w-5" />}
          trend={{
            value: stats.tarefasHoje,
            positive: stats.tarefasHoje === 0,
            label: `${formatNumber(stats.tarefasHoje)} para hoje`
          }}
        ...
        <StatsCard 
          ...
          value={formatNumber(stats.tarefasAtrasadas)} 
        ...
        <StatsCard 
          ...
          value={formatNumber(stats.execucoesRealizadas)} 
          icon={<CheckCircle className="h-5 w-5 text-green-500" />}
          trend={{
            value: stats.paradasConcluidas,
            positive: true,
            label: `${formatNumber(stats.paradasConcluidas)} paradas concluídas`
          }}
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-2 mb-6">
        ...
        <Card>
          ...
          <CardContent className="px-2 sm:px-6">
            {dadosStatusPreventivas.length === 0 ? <EmptyChart message="Nenhuma preventiva registrada" /> : (
            <div className={CHART_HEIGHT}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip {...tooltipStyle} formatter={(value, name) => [formatNumber(Number(value)), name]} />
        ...
        <Card>
          ...
          <CardContent className="px-2 sm:px-6">
            {dadosStatusParadas.length === 0 ? <EmptyChart message="Nenhuma parada registrada" /> : (
            <div className={CHART_HEIGHT}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie 
