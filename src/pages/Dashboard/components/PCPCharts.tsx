...
const formatNumber = (value: number) => new Intl.NumberFormat('pt-BR').format(Number.isFinite(value) ? value : 0);
const CHART_HEIGHT = "h-[260px] sm:h-[300px]";

const EmptyChart = ({ message = "Sem dados registrados no período" }: { message?: string }) => (
  <div className={`${CHART_HEIGHT} flex items-center justify-center text-center text-sm text-muted-foreground px-4`}>
    {message}
  </div>
);
...
export function PCPCharts({ period }: PCPChartsProps) {
  ...
  const dadosEficienciaPorTurno = useMemo(() => {
    ...
    return Object.entries(turnoMap).filter(([, data]) => data.planejado > 0 || data.produzido > 0).map(([name, data]) => ({
      name,
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4 mb-6">
        <StatsCard 
          ...
          value={`${formatNumber(metrics.producaoTotal)} kg`} 
        ...
        <StatsCard 
          ...
          value={`${formatNumber(eficienciaMedia)}%`} 
        ...
        <StatsCard 
          title="1° Turno" 
          value={`${formatNumber(Number((metrics.producaoPorTurno['1° Turno'] as any)?.quantidade || 0))} kg`} 
        ...
        <StatsCard 
          title="2° Turno" 
          value={`${formatNumber(Number((metrics.producaoPorTurno['2° Turno'] as any)?.quantidade || 0))} kg`} 
          icon={<Clock className="h-5 w-5" />}
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-2 mb-6">
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs sm:text-sm">Distribuição entre turnos</CardDescription>
          </CardHeader>
          <CardContent className="px-2 sm:px-6">
            {dadosProducaoPorTurno.length === 0 ? <EmptyChart message="Nenhuma produção registrada no período" /> : (
            <div className={CHART_HEIGHT}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip {...tooltipStyle} formatter={value => [`${formatNumber(Number(value))} kg`, 'Produção']} />
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs sm:text-sm">Comparativo por turno</CardDescription>
          </CardHeader>
          <CardContent className="px-2 sm:px-6">
            {dadosEficienciaPorTurno.length === 0 ? <EmptyChart /> : (
            <div className={CHART_HEIGHT}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dadosEficienciaPorTurno} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
                  <XAxis dataKey="name" tick={{ fontSize: 11 }} />
                  <YAxis tick={{ fontSize: 11 }} width={52} tickFormatter={(value) => formatNumber(Number(value))} />
                  <Tooltip 
                    {...tooltipStyle} 
                    formatter={(value: number, name: string, props: any) => [
                      `${formatNumber(Number(value))} kg (${formatNumber(props.payload.eficiencia)}%)`,
                      name
                    ]}
                  />
                  <Legend wrapperStyle={{ fontSize: 11 }} />
